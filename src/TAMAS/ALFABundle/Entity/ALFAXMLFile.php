 ...
 * XMLFile
 *
 * @ORM\Table(name="xml_file")
 * @ORM\Entity(repositoryClass="TAMAS\ALFABundle\Repository\ALFAXMLFileRepository")
 * @UniqueEntity(
 *     fields={"fileUserName"},
 *     errorPath="fileUserName",
 *     message="This name of xml is already taken"
 * )
 * @Vich\Uploadable
 */
class ALFAXMLFile
{
     ...
     * 
     * @Vich\UploadableField(mapping="xml_file", fileNameProperty="fileName", size="fileSize", originalName="fileOriginalName")
     * @var File
     */
    private $xmlFile;
    ...
    */
     ...
     */
    public function setXMLFile($file = null)
    {
        $this->xmlFile = $file;
    ...
    public function validateFields(ExecutionContextInterface $context) {
        if ($this->xmlFile && !in_array($this->xmlFile->getMimeType(), ['application/xml', 'text/xml'])) {
            $context
                ->buildViolation('Wrong file type (expected type: xml; uploaded type: '. $this->xmlFile->getMimeType().")")
                ->atPath('xmlFile')
                ->addViolation()
            ;
        }
